...
const Borrowers = () => {
  ...
  const handleCreateBorrower = async (e) => {
    ...
    try {
      ...
      const status = calculateStatus(newBorrower.dueDate);
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleCreateBorrower} className="p-6 space-y-6">
                ...
                {/* Status Preview - Shows what status will be set */}
                {newBorrower.dueDate && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    <div className="flex items-center gap-2 text-sm">
                      <span className="text-slate-700 font-medium">Status will be set to:</span>
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${statusColor[calculateStatus(newBorrower.dueDate)]}`}>
                        {calculateStatus(newBorrower.dueDate)}
                      </span>
